...
#import glob   
#path = '/home/va/work/HMI/hmi_synoptic_maps/vectormag/hmi.B_synoptic_small.*.Bt.fits'
#files=glob.glob(path)   

CR=range(2097,2181)
#i0=68
#image_data = nan_to_num(fits.getdata(fim+str(CR[i0])+'.Br.fits', ext=0))
#data, header = fits.getdata(fim+str(CR[i0])+'.Br.fits, header=True)
#hdul = fits.open(fim+str(CR[i0])+'.Br.fits')
LM=len(CR)
#time=linspace(2010.5,2016.94,LM+2)
NT0=360
Nph0=720
...
c3=sum(ones(Nph0)*simps(ones(NT0),(mu0)))/Nph0
...
xi,yi=meshgrid(phi*180/pi,90-arccos(mu)*180/pi)
...
#filt=append(ones(lmaxc),zeros(Nch-lmaxc))
#tfc=dot(m0c0,dot(diag(filt),m_0c0))
#tfb=dot(ma0,dot(diag(filt),ma_0))

from scipy.ndimage.filters import uniform_filter,uniform_filter1d,maximum_filter,minimum_filter
from scipy.ndimage.filters import gaussian_filter,convolve,convolve1d

from scipy.signal import detrend
def gauss_kern(sizex):
    sizex = int(sizex)
    x = mgrid[-sizex:sizex+1]
    g= exp(-x**2/(2*float(sizex)**2))
    return g / g.sum()
...
for i in CR:
    ...
    imsm = uniform_filter(f(mu,phi), size=[4,8], mode=['reflect','wrap'])
    ...
    #    brcc=-dot(ms0,s0r)
    #    cilm[0,nk,mk]=real(s0r);cilm[1,nk,mk]=imag(s0r) 
    #    rcilm=SHctor(cilm)
    #    brnr = MakeGridGLQ(rcilm,zero,norm=4)
    #        
    #    plt.figure()
    #    #plt.title('CR='+str(CR[i0]))    
    #    #CS0=plt.pcolor(x0,y0,BR,cmap=cmap.gray,vmin=-10,vmax=10)
    #    #CS0=plt.pcolor(xi,yi,imsm,cmap=cmap.seismic,vmin=-100,vmax=100,alpha=0.35)
    #    CS0=plt.pcolor(xg,yg,arm,cmap=cmap.seismic,vmin=-1,vmax=1)#,alpha=0.35)
    #    divider = make_axes_locatable(plt.gca())
    #    cax0 = divider.append_axes("right", "3%", pad="3%")
    #    CB0=plt.colorbar(CS0, cax=cax0,ticks=[-.50,0,.50])
    #    CB0.set_label('S',fontsize=30)
    #    plt.tight_layout()
    #    plt.show()
    #
    imff=nan_to_num(fits.getdata(fim+str(CR[i0])+'.Bt.fits',ext=0))#,kernel_size=[11,21]
    #imff=(imff[:,:]+imff[-1::-1,:])/2.
    f = RectBivariateSpline(mu0,phi0,imff)
    imsm = uniform_filter(f(mu,phi), size=[4,8], mode=['reflect','wrap'])
    #imsm = convolve1d(convolve1d(f(mu,phi), g1, axis=1, mode='wrap'),
    #                    g0,axis=0,mode='reflect')   
    cilm = SHExpandDH(imsm[-1::-1,:],norm=4,sampling=2,lmax_calc=Nlr)
    cilmc = SHCoeffs.from_array(cilm)
    ...
    f = RectBivariateSpline(mu0,phi0, imff)
    imsm = uniform_filter(f(mu,phi), size=[4,8], mode=['reflect','wrap'])
    #imsm = uniform_filter1d(uniform_filter1d(f(mu,phi), size=20, axis=1, mode='wrap'),
    #                    size=10,axis=0,mode='reflect') 
    cilm = SHExpandDH(imsm[-1::-1,:],norm=4,sampling=2,lmax_calc=Nlr)
    cilmc = SHCoeffs.from_array(cilm)
    ...
    hilmr = SHExpandGLQ(hlnr,wch,zero, norm=4)
    ...
    hhcr[:,i0] = SHAdmitCorr (hilmr, hcilm)[2]
    #hbsp = cross_spectrum(bilm,hilm, normalization='ortho', unit='per_l')
    ...
    #CS2=plt.pcolor(xg,yg,hel,cmap=cmap.seismic,vmin=-10,vmax=10,alpha=0.3)
    divider = make_axes_locatable(plt.gca())
    cax0 = divider.append_axes("right", "3%", pad="3%")
    ...
    plt.savefig(fima+'BHMI'+str(i0)+'.png',format='png',dpi=150);
    plt.close('all')
    ...
    #               cmap=cmap.seismic,vmin=-100,vmax=100)#,alpha=0.35)
    #CS2=plt.pcolor(xg,yg,hel,cmap=cmap.seismic,vmin=-10,vmax=10,alpha=0.3)
    divider = make_axes_locatable(plt.gca())
    cax0 = divider.append_axes("right", "3%", pad="3%")
    #CB0=plt.colorbar(CS2, cax=cax0,ticks=[-1,0,1])
    ...
    plt.savefig(fima+'h'+str(i0)+'.png',format='png',dpi=150);
    plt.close('all')
    ...
    subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.8, top=0.89)
    plt.savefig(fima+'fs'+str(i0)+'.png',format='png',dpi=150);
    plt.close('all')
    ...
    #               cmap=cmap.seismic,vmin=-10,vmax=10)#,alpha=0.35)
    #CS2=plt.pcolor(xg,yg,hel,cmap=cmap.seismic,vmin=-10,vmax=10,alpha=0.3)
    divider = make_axes_locatable(plt.gca())
    cax0 = divider.append_axes("right", "3%", pad="3%")
    #CB0=plt.colorbar(CS2, cax=cax0,ticks=[-1,0,1])
    ...
    subplots_adjust(hspace=0,bottom=0.1, left=0.1, right=0.8, top=0.89)
    plt.savefig(fima+'hc'+str(i0)+'.png',format='png',dpi=150);
    plt.close('all')
...
plt.ylabel('$\\ell$')
#CS2=plt.contour(xc,yc,hLt,levs)
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
CB0=plt.colorbar(CS, cax=cax,ticks=[0,.5,1])
plt.tight_layout()
subplots_adjust(top=0.975,bottom=0.1,left=0.1,right=0.9,hspace=0.0,wspace=0.2)
#plt.savefig(fima+'h'+str(i0)+'.png',format='png',dpi=150);
plt.show()
...
hMss=uniform_filter(hM, size=[2,10], mode=['reflect','nearest'])

plt.figure(figsize=(10,5))
CS=plt.pcolor(xc,yc,hMss,cmap=cmap.seismic,vmin=-20,vmax=20)
#CS2=plt.contour(xc,yc,hLt,levs)
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
...
hSss=uniform_filter(hS, size=[2,10], mode=['reflect','nearest'])

plt.figure(figsize=(10,5))
CS=plt.pcolor(xc,yc,hSss,cmap=cmap.seismic,vmin=-5,vmax=5)
#CS2=plt.contour(xc,yc,hLt,levs)
divider = make_axes_locatable(plt.gca())
cax = divider.append_axes("right", "3%", pad="3%")
CB0=plt.colorbar(CS, cax=cax,ticks=[-2,0,2])
...
plt.figure()
plt.plot(CR,uniform_filter1d(htN,size=10),'b',CR,uniform_filter1dsmotthing(htS,size=10),'r',
         CR,uniform_filter1d(hsN*10,size=10),'b--',CR,uniform_filter1d(hsS*10,size=10),'r--')
subplots_adjust(top=0.975,bottom=0.1,left=0.1,right=0.85,hspace=0.0,wspace=0.2)
plt.show()


plt.figure()
plt.plot(CR,uniform_filter1d(hT,6),'b',CR,uniform_filter1d(hTl,6)*10,'r')
subplots_adjust(top=0.975,bottom=0.1,left=0.1,right=0.85,hspace=0.0,wspace=0.2)
plt.show()
